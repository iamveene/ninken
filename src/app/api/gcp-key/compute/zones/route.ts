...
/**
 * GET /api/gcp-key/compute/zones
 * Lists available Compute Engine zones for the project.
 */
export async function GET(req: NextRequest) {
  ...
  try {
    const data = await gcpKeyFetch<{ items?: unknown[] }>({
      credential,
      url: `https://compute.googleapis.com/compute/v1/projects/${project}/zones`,
    })
    return NextResponse.json({ zones: data.items ?? [] })
  } catch (err) {
