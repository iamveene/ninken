...
export default function GoogleDashboardPage() {
  ...
  // Use Google's scopeAppMap directly (not useScopes().hasApp which uses the active provider's map)
  ...
  const providerMismatch = provider !== "google"
    ...
    <div className="space-y-6">
      ...
      {providerMismatch && (
        <div className="flex items-center gap-2 rounded-lg border border-amber-500/30 bg-amber-950/10 px-3 py-2 text-sm text-amber-200">
          ...
          <span>
            Active session is <span className="font-medium">{provider}</span> — scope data below reflects the active token, not Google Workspace.
          </span>
        </div>
      )}
