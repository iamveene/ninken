...
type FilterKey = "all" | "failed" | "risky" | "mfa"
...
export default function SignInLogsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const mfa = signIns.filter((r) => r.conditionalAccessStatus === "success").length
    return { total, failed, risky, mfa }
  }, [signIns])
  ...
  const filters: { key: FilterKey; label: string; count?: number }[] = [
    ...
    { key: "mfa", label: "CA Enforced", count: stats.mfa },
  ]
        ...
        <>
          {!loading && signIns.length > 0 && (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <Card>
                <CardContent className="py-3">
                  <p className="text-xs text-muted-foreground">CA Enforced</p>
                  <p className="text-2xl font-bold">{stats.mfa}</p>
                </CardContent>
              </Card>
            </div>
          )}
