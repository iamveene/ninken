...
const [showCapabilities, setShowCapabilities] = useState(false)
  ...
  useEffect(() => {
    async function init() {
      // Handle NinLoader CLI import: ?import=CODE
      // When importCode is present, skip existing profile redirect entirely —
      // the import flow handles its own redirect via submitToken.
      if (importCode) {
        try {
          const res = await fetch(`/api/auth/import?code=${importCode}`)
          if (res.ok) {
            const { payload } = await res.json()
            if (payload) {
              setMigrating(false)
              // Clean the ?import=CODE from the URL immediately
              router.replace("/")
              // Call submitToken directly — it handles profile creation,
              // activation, capability probing, and redirect to the
              // correct provider dashboard.
              await submitToken(JSON.stringify(payload))
              return
            }
          }
        } catch {
          // Import failed — fall through to normal init
        }
        // Import code was invalid/expired — clean URL and show landing page
        router.replace("/?add=true")
      }

      // Normal init: check for existing profiles and redirect
      try {
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex justify-center gap-2">
            {providers.map((p) => {
              ...
              const isTest = testProviders.has(p.id)
              return (
                <button
                  key={p.id}
                  type="button"
                ...
                >
                  ...
                  {isTest && (
                    <span className="absolute -top-1.5 -right-1.5 rounded-full bg-yellow-600 px-1.5 py-0.5 text-[9px] font-bold leading-none text-black">
                      Test
                    </span>
                  )}
                </button>
