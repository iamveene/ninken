...
export default function GitHubRiskDashboardPage() {
  const { assessment, data, loading, error } = useGitHubAuditRisk(true)
    ...
    <div className="flex flex-col gap-6">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">GitHub Risk Dashboard</h1>
        </div>
        <p className="text-sm text-muted-foreground mt-1">
          Risk assessment across repositories, branch protections, Actions runners, deploy keys, and webhooks.
        </p>
      </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          {(data?.skippedRepos ?? 0) > 0 && (
            <div className="flex items-center gap-2 rounded-lg border border-amber-500/30 bg-amber-950/10 px-3 py-2 text-sm text-amber-200">
              ...
              <span>{data!.skippedRepos} repo{data!.skippedRepos! > 1 ? "s" : ""} skipped due to errors. Risk assessment may be incomplete.</span>
            </div>
          )}
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-2 py-8 text-center">
            ...
            <p className="text-xs text-muted-foreground">
              Ensure a GitHub token with repo and admin:org scopes is active, then reload the page.
            </p>
          </CardContent>
        </Card>
      )}
