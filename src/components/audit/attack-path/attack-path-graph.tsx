...
import { M365_NODE_COLORS } from "@/lib/audit/m365-attack-path-builder"
import { AlertTriangle } from "lucide-react"
import { Badge } from "@/components/ui/badge"

// Merge Google + M365 node colors for the MiniMap
const ALL_NODE_COLORS: Record<string, string> = { ...NODE_COLORS, ...M365_NODE_COLORS }
...
const nodeTypes: NodeTypes = {
  ...
  "service-principal": ServiceAccountNode,  // reuse SA visual
  domain: DomainNode,
  tenant: DomainNode,                       // reuse domain visual
  application: ApplicationNode,             // new
}
...
function buildReactFlowData(
  ...
  const tenantNodes = apNodes.filter((n) => n.type === "tenant")
  const roleNodes = apNodes.filter((n) => n.type === "role")
  const saNodes = apNodes.filter((n) => n.type === "service-account")
  const spNodes = apNodes.filter((n) => n.type === "service-principal")
  const appNodes = apNodes.filter((n) => n.type === "application")
  const groupNodes = apNodes.filter((n) => n.type === "group")
  const userNodes = apNodes.filter((n) => n.type === "user")

  // Layered layout: domain/tenant at top, then roles, groups, service-accounts/principals/apps, users at bottom
  const layers: { nodes: APNode[]; y: number }[] = [
    { nodes: [...domainNodes, ...tenantNodes], y: 0 },
    { nodes: roleNodes, y: 180 },
    { nodes: groupNodes, y: 360 },
    { nodes: [...saNodes, ...spNodes, ...appNodes], y: 540 },
      ...
      >
        ...
        <MiniMap
          ...
          nodeColor={(node) => {
            ...
            return t && t in ALL_NODE_COLORS ? ALL_NODE_COLORS[t] : "#6B7280"
          }}
