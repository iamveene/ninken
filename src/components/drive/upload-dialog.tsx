...
import { useUploadFile } from "@/hooks/use-drive"
...
export function UploadDialog({ folderId, onComplete, children }: UploadDialogProps) {
  ...
  const handleUploadAll = async () => {
    for (let i = 0; i < items.length; i++) {
      ...
      const result = await upload(items[i].file, folderId)
