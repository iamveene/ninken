...
export function ObjectBrowser({ bucket, onBackToBuckets }: ObjectBrowserProps) {
  ...
  const { objects, prefixes, loading, error: objectsError, accessDenied, canDownload, refetch } = useObjects(bucket, prefix || undefined)
  ...
  return (
    <div className="flex h-full">
      ...
      >
        ...
        {loading ? (
          <BrandedLoader />
        ) : objectsError && !accessDenied ? (
          <ErrorState message={objectsError} onRetry={refetch} />
        ) : accessDenied ? (
          <AccessDeniedState />
...
function ErrorState({ message, onRetry }: { message: string; onRetry: () => Promise<void> }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-4 py-20">
      ...
      <div className="text-center">
        <p className="text-lg font-medium">Failed to load objects</p>
