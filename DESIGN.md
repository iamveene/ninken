...
| **Google Admin Reports API** | High | **Implemented** | Admin SDK Reports API audit module. Activity reports for login events, admin actions, OAuth token grants, drive events, mobile events. 5 application types, time range filtering, user filtering, OPSEC warning banner. Files: `src/app/api/audit/admin-reports/`, `src/app/(google)/audit/admin-reports/`, `src/hooks/use-audit-admin-reports.ts`. |
| **Google Alert Center** | High | **Implemented** | Alert Center API module. Security alerts (phishing, suspicious login, government-backed attacks, leaked passwords). Severity classification (Critical/High/Medium/Low), expandable alert detail with raw JSON, OPSEC detection awareness. Files: `src/app/api/audit/alert-center/`, `src/app/(google)/audit/alert-center/`, `src/hooks/use-audit-alert-center.ts`. |
| **Google Drive Activity API** | High | **Implemented** | Drive Activity v2 API module. File access/modify/share tracking. Action type filters, permission change highlighting, external share detection. Files: `src/app/api/audit/drive-activity/`, `src/app/(google)/audit/drive-activity/`, `src/hooks/use-audit-drive-activity.ts`. |
| **Google Groups Settings** | High | **Implemented** | Groups Settings API module. Security configuration audit for all groups. Risk scoring (CRITICAL/HIGH/MEDIUM/LOW), external member detection, join/post policy analysis. Files: `src/app/api/audit/groups-settings/`, `src/app/(google)/audit/groups-settings/`, `src/hooks/use-audit-groups-settings.ts`. |
| **Google Contacts/People API** | High | **Implemented** | People API module. Org directory enumeration, personal contacts, other contacts. Three data sources with graceful fallback. Quieter than admin Directory API. Files: `src/app/api/audit/contacts/`, `src/app/(google)/audit/contacts/`, `src/hooks/use-audit-contacts.ts`. |
| **MS CA Policy Enhancement** | High | **Implemented** | Enhanced Conditional Access viewer with human-readable conditions, named location resolution, grant control badges, coverage gap analysis. Files: modified `src/app/api/microsoft/audit/conditional-access/`, `src/app/(microsoft)/m365-audit/conditional-access/`, `src/hooks/use-m365-audit.ts`. |
| **MS Authentication Methods** | High | **Implemented** | Per-user MFA method enumeration. Aggregate stats (password-only, phone-only, FIDO2, Authenticator), per-user drill-down. Files: `src/app/api/microsoft/audit/auth-methods/`, `src/app/(microsoft)/m365-audit/auth-methods/`. |
| **MS Identity Protection Enhancement** | High | **Implemented** | Enhanced risky users with risk detections tab, per-user investigation, risk event timeline. Files: `src/app/api/microsoft/audit/risk-detections/`, modified `src/app/(microsoft)/m365-audit/risky-users/`. |
| **MS Cross-Tenant Access** | High | **Implemented** | Cross-tenant access policy enumeration. Default policy + partner overrides with inbound/outbound trust badges. Files: `src/app/api/microsoft/audit/cross-tenant/`, `src/app/(microsoft)/m365-audit/cross-tenant/`. |
| **Resource Pivot Probing** | High | **Implemented** | Probe ARM, Key Vault, Azure Storage, Azure DevOps access from a Microsoft credential. Token exchange with different resource scopes. Files: `src/app/api/microsoft/audit/resource-pivot/`, `src/app/(microsoft)/m365-audit/pivot/`. |
| **FOCI Client ID Pivoting** | High | **Implemented** | Batch probe across 15 FOCI client IDs. Scope discovery matrix, new-scope highlighting. Manual trigger with OPSEC warning. Files: `src/app/api/microsoft/audit/foci-pivot/`, `src/app/(microsoft)/m365-audit/foci-pivot/`, `src/hooks/use-foci-pivot.ts`. |
| **Azure CLI/PS Cache Import** | High | **Implemented** | Multi-account extraction from Azure CLI MSAL cache and PowerShell context files. Account selection UI with "Import Selected"/"Import All". Files: modified `src/lib/providers/microsoft.ts`, `src/app/page.tsx`. |
| **gcloud ADC Format Detection** | High | **Implemented** | Support for Application Default Credentials format, nested `installed`/`web` wrappers, `type: "authorized_user"` detection. Service account key rejection with clear error. Files: modified `src/lib/providers/google.ts`. |
| **Cookie Size Solution** | High | **Implemented** | Server-side session store for credentials exceeding 4KB cookie limit. Two-tier storage: direct cookie for small credentials, session reference for large ones. Files: `src/lib/session-store.ts`, modified `src/app/api/auth/activate/route.ts`, `src/app/api/_helpers.ts`. |
| **CredentialStrategy Pattern** | High | **Implemented** | Extensible strategy pattern for credential handling. Each credential kind (OAuth, FOCI, service-account, service-principal, access-token) is a standalone strategy. Files: `src/lib/providers/credential-strategy.ts`, `src/lib/providers/strategies/`. |
| **Service Account Keys (Google)** | High | **Implemented** | Google Cloud service account key support with JWT assertion flow using Web Crypto API. Files: `src/lib/providers/strategies/google-service-account.ts`. |
...
| Google OPSEC Intelligence (Phase 1) | multi-agent | 5 new Google audit modules: Admin Reports API (login/admin/OAuth/drive/mobile events), Alert Center (phishing/suspicious login/government attacks), Drive Activity v2 (file access/modify/share tracking), Groups Settings (risk scoring, external member detection), Contacts/People API (directory enumeration, 3 data sources). |
| Microsoft OPSEC Intelligence (Phase 1) | multi-agent | 4 enhanced/new M365 audit modules: CA Policy Enhancement (human-readable conditions, gap analysis), Authentication Methods (per-user MFA enumeration), Identity Protection Enhancement (risk detections tab, timeline), Cross-Tenant Access (default + partner policies). |
| Microsoft Credential Capabilities (Phase 1) | multi-agent | 3 credential capability modules: Resource Pivot Probing (ARM, Key Vault, Storage, DevOps), FOCI Client ID Pivoting (15 client IDs, scope matrix), Azure CLI/PS Cache Import (MSAL cache + PowerShell context extraction). |
| Google Credential Import Enhancement | multi-agent | gcloud ADC format detection: Application Default Credentials, nested installed/web wrappers, authorized_user type, service account key rejection. |
| New Credential Types (Phase 2) | multi-agent | 6 features: Cookie Size Solution (server-side session store), CredentialStrategy Pattern (extensible strategy per credential kind), Service Account Keys (Google JWT assertion), Service Principal Auth (Microsoft client_credentials), Raw Access Token Ingestion (Google ya29.*/Microsoft JWT), Capability Probing on Import (auto-probe with accessibility grid). |
